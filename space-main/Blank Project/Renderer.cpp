...
Renderer::Renderer(Window& parent) :OGLRenderer(parent)
{
	...
	palmtree = Mesh::LoadFromMeshFile("SP_Tree.msh");	
	palmtree01 = Mesh::LoadFromMeshFile("SP_Planet.msh");
	palmtree02 = Mesh::LoadFromMeshFile("SP_Crystal01.msh");		//Tree "https://opengameart.org/content/free-palm-treez-v3", "https://assetstore.unity.com/?category=3d%2Fvegetation%2Ftrees&free=true&orderBy=1"
	npc = Mesh::LoadFromMeshFile("spider.msh");
	...
	//Animation
	anim = new MeshAnimation("spider.anm");
	material = new MeshMaterial("spider.mat");
	...
	//	s->SetTransform(Matrix4::Translation(dimensions * Vector3(0.5f * i, 0.05f, 0.7f * i)));//1?2z3?
	...
	for (int i = 1; i < 4; ++i)
	{
		...
		s->SetTransform(Matrix4::Translation(dimensions * Vector3(0.4f * i, 0.5f, 0.3f * i)));
		s->SetModelScale(Vector3(200.0f, 200.0f, 200.0f));
		s->SetMesh(palmtree02);
		s->SetTexture(treeTex);
		root->AddChild(s);
	}
	...
	for (int i = 1; i < 3; ++i)
	{
		SceneNode* s = new SceneNode();
		s->SetColour(Vector4(1.0f, 1.0f, 1.0f, 1.0f));
		s->SetTransform(Matrix4::Translation(dimensions * Vector3(0.3f * i, 10.5f, 0.7f * i)));
		s->SetModelScale(Vector3(100.0f, 100.0f, 100.0f));
		s->SetMesh(palmtree01);
		s->SetTexture(treeTex);
		root->AddChild(s);
	}
	...
	for (int i = 1; i < 4; ++i)
	{
		SceneNode* s = new SceneNode();
		s->SetColour(Vector4(1.0f, 3.0f, 5.0f, 1.0f));
		s->SetTransform(Matrix4::Translation(dimensions * Vector3(0.8f * i, 0.08f, 0.45f * i)));
		s->SetModelScale(Vector3(100.0f, 100.0f, 100.0f));
		s->SetMesh(palmtree);
		s->SetTexture(treeTex);
		root->AddChild(s);
